...
!
! Version - @(COHERENS)switches.f90  V2.12.1
!
...
!---grid
INTEGER :: iopt_grid_htype = 1, iopt_grid_nodim = 3, iopt_grid_sph = 1, &
...
!---sediments
INTEGER ::  iopt_dar = 0, iopt_morph = 0, iopt_sed = 0
!---particle module
...
!---diffusion
INTEGER ::  iopt_hdif_coef = 0, iopt_hdif_lim = 1, iopt_hdif_scal = 0, &
          & iopt_hdif_skew = 0, iopt_hdif_turb = 0, iopt_hdif_2D = 0, &
          & iopt_hdif_3D = 0, iopt_kinvisc = 0, iopt_vdif_coef = 3, &
          & iopt_vdif_rot = 1
!---turbulence
INTEGER :: iopt_turb_alg = 1, iopt_turb_dis_bbc = 2, iopt_turb_dis_sbc = 2, &
         ...
         & iopt_turb_tke_sbc = 2
!---drying/wetting
INTEGER :: iopt_fld = 0, iopt_fld_alpha = 1, iopt_fld_crit = 1
!---structures
INTEGER :: iopt_dischr = 0, iopt_dischr_land = 1, iopt_drycel = 0, &
         & iopt_thndam = 0, iopt_weibar = 0, iopt_mpvcov = 0
!---open boundary conditions
INTEGER :: iopt_obc_advrlx = 0, iopt_obc_bio = 0, iopt_obc_invbar = 0, &
         & iopt_obc_sal = 0, iopt_obc_sed = 0, iopt_obc_temp = 0, &
         & iopt_obc_th = 0, iopt_obc_2D = 0, iopt_obc_2D_tang = 0, &
         & iopt_obc_3D = 0, iopt_obc_3D_tang = 0
!---astronomical tide
INTEGER :: iopt_astro_pars = 2, iopt_astro_tide = 0
!---1-D applications
INTEGER:: iopt_sbc_1D = 0, iopt_sur_1D = 0
...
INTEGER :: iopt_MPI, iopt_MPI_abort = 0, iopt_MPI_comm_all = 2, &
         ...
         & iopt_MPI_comm_gath = 2, iopt_MPI_comm_scat = 2, &
         & iopt_MPI_partit = 1, iopt_MPI_sync = 0 
!---output
...
!---multigrid
INTEGER ::  iopt_mg_cycle = 1, iopt_mg_prolong = 2, iopt_mg_smoother = 1
!---random generator
INTEGER :: iopt_rng_seed = 1
...
!*iopt_astro_pars*   INTEGER Type of evaluation of astronomical phases
!                     = 0 => Disabled
!                     = 1 => Initial astronomical argument defined by the user,
!                            no nodal corrections
!                     = 2 => Initial astronomical argument defined by the user,
!                            nodal corrections updated at time intervals
!                            (in days) selected with the noastrodays time
!                            counter. 
...
!                     = 5 => form roughness formulation (sum of grain,
!                            wave ripple and sediment transport comonents) 
...
!*iopt_CDF_sync*     INTEGER Disables/enables synchronisation of the disk copy
!                            of a netCDF user output file with in-memory buffers
...
!*iopt_fld*          INTEGER Selects drying/wetting algorithm (0/1)
...
!*iopt_fld_crit      INTEGER Type of mask criterion for defining temporarily
!                            dry grid cells
!                     = 0 => disabled
!                     = 1-4
...
!*iopt_grid_vtype_transf*
!                    INTEGER Type of transformation for vertical grid
!                     = 0  => not used
!                     = 20 => user-defined
!                     = 21 => log-transformation at the bottom following Davies
!                             and Jones (1991) if iopt_grid_vtype=2
!                     = 22 => log-transformation at the surface following Davies
!                             and Jones (1991) if iopt_grid_vtype=2
!                     = 23 => Burchard and Bolding (2002) if iopt_grid_vtype=2
!                     = 30 => user defined
!                     = 31 => Song and Haidvogel (1994) if iopt_grid_vtype=3
!                     = 32 => Burchard and Bolding (2000) if iopt_grid_vtype=3
...
!*iopt_hdif_skew*    INTEGER Disables/enables adding the Griffies (1998) skew
!                            diffusive flux in the diffusion tensor if
!                            iopt_hdif_scal = 3 (0/1).
...
!*iopt_meteo_pres*   INTEGER Disables/enables input of atmospheric
!                            pressure (0/1)
...
!                            salinity
!                     = 0 => default conditions at all points
!                     = 1 => non-default conditions for at least one point
...
!                     = 3 => particle module runs offline in forward mode and
!                            reads data from previous COHERENS run
!                     = 4 => particle module runs offline in backward mode and
!                            reads data from previous COHERENS run
...
!                            for all simulations
!                     = 2 => The initial seeds are generated depending on
!                            daytime (different runs produce different series
!                            of random numbers)   
...
!*iopt_sbc_1D*       INTEGER Type of data in 1-D surface data file
!                     = 0 => no data provided
...
!                     = 2 => dissipation rate
!*iopt_turb_stab_form*
!                    INTEGER Selects form of stability functions
!                     = 1 => constant
...
!                            equations (0/1)
!*iopt_waves_extrapol*
!                    INTEGER Disables/enables extrapolation if the model/data
!                            grid points are outside the data/model
!                            domain (0/1).
!*iopt_waves_form*   INTEGER Selects evaluation of wave parameters
!                     = 1 => using significant wave height, peak wave number
!                            and wave direction
