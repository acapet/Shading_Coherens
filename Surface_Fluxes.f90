...
!
! Version - @(COHERENS)Surface_Fluxes.f90  V2.12.1
!
! $Date: 2021-11-05 11:33:25 +0100 (Fr, 05 Nov 2021) $
!
!
! $Revision: 1408 $
!
...
!
INTEGER :: npcc
REAL, PARAMETER :: cpair = 1005.0, emiscoef = 0.985
...
!
! External calls - define_surface_input_grid, exchange_coefs,
!                  update_surface_data
!
! Module calls - error_alloc, error_alloc_struc, exchange_mod,
!                hrelativecoords_init, rotate_vec
!
!************************************************************************
!
USE datatypes
...
!
!2.2 Number of and ids of meteo variables
!----------------------------------------
!
   ...
   IF (iopt_meteo_stres.EQ.1) THEN
      IF (iopt_meteo_data.EQ.1) THEN
         ivar = ivar +1; imet_uwindatc = ivar
         ivar = ivar +1; imet_vwindatc = ivar
      ELSEIF (iopt_meteo_data.EQ.2) THEN
         ivar = ivar +1; imet_usstresatc = ivar
         ivar = ivar +1; imet_vsstresatc = ivar
      ENDIF
   ENDIF
   IF (iopt_meteo_pres.EQ.1) THEN
      ivar = ivar + 1; imet_atmpres = ivar
   ENDIF
   IF (iopt_meteo_heat.EQ.1) THEN
      IF (iopt_meteo_data.EQ.1) THEN
         ivar = ivar + 1; imet_airtemp = ivar
         ivar = ivar + 1; imet_relhum = ivar
         ivar = ivar + 1; imet_cloud_cover = ivar
      ELSEIF (iopt_meteo_data.EQ.2) THEN
         ivar = ivar + 1; imet_qnonsol = ivar
         ivar = ivar + 1; imet_qrad = ivar
      ENDIF
   ENDIF
   IF (iopt_meteo_precip.EQ.1) THEN
      ivar = ivar + 1; imet_precipitation = ivar
   ELSEIF (iopt_meteo_precip.EQ.2) THEN
      ivar = ivar + 1; imet_evapminprec = ivar
   ENDIF
   novars = ivar
...
!---------------------------------
!
!5.1 Wind components or surface stress
!-------------------------------------
!
!--store data
ivar_510: DO ivar=1,novars
   IF (ivar.EQ.imet_uwindatc) THEN
      uwindatc(1:ncloc,1:nrloc) = meteovals(:,:,ivar)
   ELSEIF (ivar.EQ.imet_vwindatc) THEN
      vwindatc(1:ncloc,1:nrloc) = meteovals(:,:,ivar)     
   ELSEIF (ivar.EQ.imet_usstresatc) THEN
      usstresatc(1:ncloc,1:nrloc) = meteovals(:,:,ivar)
   ELSEIF (ivar.EQ.imet_vsstresatc) THEN
      vsstresatc(1:ncloc,1:nrloc) = meteovals(:,:,ivar)
   ENDIF
ENDDO ivar_510

!---case of rotated grid
IF (iopt_meteo_stres.EQ.1.AND.rotate_gvecs) THEN
   IF (iopt_meteo_data.EQ.1) THEN
      CALL rotate_vec(meteovals(:,:,imet_uwindatc),&
                    & meteovals(:,:,imet_vwindatc),&
                    & uwindatc(1:ncloc,1:nrloc),vwindatc(1:ncloc,1:nrloc),&
                    & gangleatc(1:ncloc,1:nrloc),1)
   ELSEIF (iopt_meteo_data.EQ.2) THEN
      CALL rotate_vec(meteovals(:,:,imet_usstresatc),&
                    & meteovals(:,:,imet_vsstresatc),&
                    & usstresatc(1:ncloc,1:nrloc),vsstresatc(1:ncloc,1:nrloc),&
                    & gangleatc(1:ncloc,1:nrloc),1)
   ENDIF
ENDIF

!
!5.2 Case when routine is called from particle module
!----------------------------------------------------
!
...
!
!5.3 Other meteo data
!--------------------
!

ivar_530: DO ivar=1,novars
   IF (ivar.EQ.imet_atmpres) THEN
      atmpres(1:ncloc,1:nrloc) = meteovals(:,:,ivar)
   ELSEIF (ivar.EQ.imet_airtemp) THEN
      airtemp = meteovals(:,:,ivar)
      sst = temp(1:ncloc,1:nrloc,nz)
   ELSEIF (ivar.EQ.imet_relhum) THEN
      relhum = meteovals(:,:,ivar)
   ELSEIF (ivar.EQ.imet_cloud_cover) THEN
      cloud_cover = meteovals(:,:,ivar)
   ELSEIF (ivar.EQ.imet_qnonsol) THEN
      qnonsol = meteovals(:,:,ivar)
   ELSEIF (ivar.EQ.imet_qrad) THEN
      qrad = meteovals(:,:,ivar)
   ELSEIF (ivar.EQ.imet_precipitation) THEN
      precipitation = meteovals(:,:,ivar)
   ELSEIF (ivar.EQ.imet_evapminprec) THEN
...
!*sinalt*     REAL    Cosinus of the sun's zenith angle
...
ALLOCATE (sinalt(ncloc,nrloc),STAT=errstat)
CALL error_alloc('sinalt',2,(/ncloc,nrloc/),kndrtype)
...
!---zenith angle
sinalt = SIN(decsol)*SIN(gylat(1:ncloc,1:nrloc))+COS(decsol)*&
       & COS(gylat(1:ncloc,1:nrloc))*COS(hoursol)
sinalt = MAX(0.0,sinalt)

!---clear sky albedo
albedo_cs = 0.05/(1.1*sinalt**1.4+0.15)
...
!  ---attenuation factor
   WHERE (sinalt.GT.0.05)
      array2dc = 0.5*tau**(1.0/sinalt) + 0.455
   ELSEWHERE
      array2dc = 0.455
   END WHERE
...
!  ---short-wave radiation
   qrad = solrad*fabsorb*sinalt*array2dc*cloudcorr
...
!  ---short-wave radiation
   qrad = solrad*fabsorb*cloudcorr*sinalt*sinalt/&
        & (1.085*sinalt+0.001*vappres_air*(2.7+sinalt)+0.1)
...
!  ---clear sky part)
   qrad = solrad*(1.0-albedo_cs)*(1.0-cloud_cover)*sinalt*sinalt/&
        & (1.2*sinalt+0.001*vappres_air*(1.0+sinalt)+0.0455)
         
!  ---add oversky part
   qrad = qrad + cloud_cover*(1.0-albedo_os)*SQRT(sinalt)*(53.5+1274.5*sinalt)/&
...
DEALLOCATE (albedo_cs,fabsorb,hoursol,sinalt)
...
IF (clev.EQ.'A') THEN
   ...
   WHERE (maskatc_int.AND.airtemp.GT.0.0)
      vpres = 6.1078*relhum*EXP(17.269*airtemp/(airtemp+273.29))
   END WHERE
   WHERE (maskatc_int.AND.airtemp.LE.0.0)
      vpres = 6.1078*relhum*EXP(21.875*airtemp/(airtemp+265.49))
   END WHERE
...
ELSEIF (clev.EQ.'S') THEN
   ...
   WHERE (maskatc_int.AND.sst.GT.0.0)
      vpres = 6.1078*EXP(17.269*sst/(sst+273.29))
   END WHERE
   WHERE (maskatc_int.AND.sst.LE.0.0)
      vpres = 6.1078*EXP(21.875*sst/(sst+265.49))
   END WHERE
