...
!
! Version - @(COHERENS)syspars.f90  V2.x
!
! $Date: 2022-01-25 09:48:44 +0100 (Di, 25 Jan 2022) $
!
! $Revision: 1458 $
!
! Description - 
...
INTEGER, PARAMETER :: lencifblock = 60, lencifline = 2000, lencifvar = 120,  &
                    & lencomm = 120, lendesc = 120, lendim = 16, &
                    & lenerrcode = 120, lenformat = 30, lenfreq = 7, &
                    & leniofile = 120, leninst = 120, lenname = 35, &
                    & lennode = 3, lenform = 1, lentime = 23, lentitle = 20, &
                    & lentype = 14, lenunit = 60, lenversion = 15

!---cif file
CHARACTER (LEN=1), PARAMETER :: cifcom = '!', cifend ='#', cifrep = '$', &
                              & cifsep =','

!---user output
LOGICAL, PARAMETER :: DegreesOut = .TRUE.
CHARACTER (LEN=lenversion), PARAMETER :: CF_version = 'CF-1.6', &
                                       & model_version = 'V2.12.1'

!---output formats
CHARACTER (LEN=lenformat), PARAMETER :: CharacterFormat='(500(A,1X))', &
                                      & IntegerFormat='(50I11)', &
                                      & LogicalFormat ='(50L1)', &
                                      & RealFormat = '(50G16.7)', &
                                      & DoubleFormat = '(50E23.14E3)'
...
!                          .TRUE.  => degrees
!                          .FALSE. => radians
!*DoubleFormat*    CHAR    Default format specification for formatted double
!                          precision real output
!*IntegerFormat*   CHAR    Default format specification for formatted integer
!                          output
!*LogicalFormat*   CHAR    Default format specification for formatted logical
!                          output
!*RealFormat*   CHAR       Default format specification for formatted simple
!                          precision real output
...
!*MaxGridFiles*    INTEGER Maximum number of surface grid files per type
!*MaxGridTypes*    INTEGER Maximum number of surface grid types
...
!*MaxIOCoords*     INTEGER Maximum number of coordinates in I/O files
!*MaxIODims*       INTEGER Maximum number of dimension used in forcing files
!*MaxIOFiles*      INTEGER Maximum number of I/O files per forcing type
!*MaxIOTypes*      INTEGER Maximum number of model I/O types
!*MaxPartArids*    INTEGER Maximum number of particle key ids
!*MaxSubVars*      INTEGER Maximum number of sub-variables 
!*MaxVarDims*      INTEGER Maximum rank of model variables
...
!*cifrep*          CHAR    Repeat count specifier
...
!*lendim*          INTEGER Length of a dimension name
...
!*lenttype*        INTEGER Length of a data type string
