...
!
! Version - @(COHERENS)Allocate_Arrays.f90  V2.12
!
...
! Author - Patrick Luyten
!
! Version - @(COHERENS)Allocate_Arrays.f90  V2.11.3
!
! Description -
!
! Reference -
...
ALLOCATE (deptotglb(0:nc+1,0:nr+1),STAT=errstat)
CALL error_alloc('deptotglb',2,(/nc+2,nr+2/),kndrtype)
deptotglb = 0.0
...
ALLOCATE (fnodal_astro(nconastro),STAT=errstat)
CALL error_alloc('fnodal_astro',1,(/nconastro/),kndrtype)
IF (nconastro.GT.0) fnodal_astro = 1.0

ALLOCATE (fnodal_obc(nconobc),STAT=errstat)
CALL error_alloc('fnodal_obc',1,(/nconobc/),kndrtype)
IF (nconobc.GT.0) fnodal_obc = 1.0
...
IF (iopt_fld.EQ.1) DEALLOCATE (alphatc_fld, alphatu_fld,alphatv_fld)
...
IF (ALLOCATED(deptotglb)) DEALLOCATE(deptotglb)
...
IF (iopt_fld.EQ.1) DEALLOCATE (deptotatc_err,deptotatc_prev)
...
DEALLOCATE (fnodal_astro,fnodal_obc)
DEALLOCATE (phase_astro,phase_obc)
DEALLOCATE (fxastro,fyastro)

IF (iopt_obc_2D.EQ.1) THEN
   DEALLOCATE (ityp2dobu,ityp2dobv,iloczobu,iloczobv)
...
DEALLOCATE (iprofobx,iprofoby,itypobx,itypoby,ityp2dobx,ityp2doby,udatoby,&
          & uveloby,vdatobx,vvelobx)
...
IF (iopt_nests.EQ.1) THEN
   DEALLOCATE (nohnstglbc,nohnstglbu,nohnstglbv,nohnstglbx,nohnstglby)
...
IF (iopt_waves.GT.0) THEN
   DEALLOCATE (wavedir,waveexcurs,wavefreq,waveheight,wavenum,waveperiod,&
             & wavevel)
   IF (iopt_waves_curr.EQ.1) THEN
...
IF (iopt_weibar.EQ.1) THEN
   DEALLOCATE (indexwbaru,indexwbarv,iwbaru,iwbaruloc,iwbarv,iwbarvloc,jwbaru,&
             & jwbaruloc,jwbarv,jwbarvloc,oricoefu,oricoefv,oriwidthu,&
             & oriwidthv,oriheightu,oriheightv,wbarcoefu,wbarcoefv,wbarcrestu,&
...
IF (iopt_out_anal.EQ.1) THEN
   ...
   DEALLOCATE (nofreqsharm,ifreqsharm,index_anal)
   DEALLOCATE (harm_freq_names,harm_freqs,phase_anal)
ENDIF
