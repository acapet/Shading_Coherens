...
!
! Version - @(COHERENS)Model_Initialisation.F90  V2.x
!
! $Date: 2021-12-29 12:16:49 +0100 (Mi, 29 Dez 2021) $
!
! $Revision: 1445 $
!
! Description - 
...
!
   ...
   global_NF90 = NF90_global; int_NF90 = NF90_int; log_NF90 = NF90_int
...
! Author - Patrick Luyten
!
! Version - @(COHERENS)Model_Initialisation.F90  V2.12.1
!
! Description -
!
! Reference -
...
!   allocate_part_arrays, allocate_sed_arrays, allocate_struc_arrays,
...
!   combine_particle_phys_data, combine_particle_grid, correct_free_surface,
!   current_corr, current_pred, current_2d, dar_init, default_grid,
!   default_phsics, define_global_grid, define_local_grid, define_2dobc_spec,
!   define_partics, define_particle_phys_data, define_phsics, depth_at_nodes,
!   discharges, domain_decomposition, drying_factor, equation_of_state,
!   exchange_bioics, exchange_sedics, flocculation_model, grid_arrays,
!   harmonic_analysis, horizontal_diff_coefs, initialise_biology_arrays,
!   initialise_multigrid, initialise_physical_arrays,
!   initialise_sediment_arrays, local_struc_arrays, meteo_input,
!   nest_locations, open_boundary_arrays, pointer_arrays,
!   particle_concentrations, particle_trajects, particle_trajects_init,
!   read_bioics, read_cif_params, read_dar_spec, read_grid, read_morphics,
!   read_part_spec, read_sedics, read_sed_spec, read_thin_dams, read_weirs,
!   salinity_equation, sediment_advdiff, sediment_equation, set_procnums,
!   surface_stress, temperature_equation, time_averages, time_series,
!   update_nest_data_2d, update_wave_params, update_tang_2dobc_data,
!   update_tang_3dobc_data, update_1dsur_data, usrdef_bioics,
!   usrdef_bio_params, usrdef_dar_params, usrdef_dar_spec, usrdef_dischr_spec,
!   usrdef_grid, usrdef_mod_params, usrdef_morphics, usrdef_morph_params,
!   usrdef_output, usrdef_part_output, usrdef_part_params, usrdef_part_spec,
...
!   wave_input, write_bioics, write_cif_vars_anal, write_cif_vars_avr,
!   write_cif_vars_bio, write_cif_vars_dar, write_cif_vars_disspc,
!   write_cif_vars_freq, write_cif_vars_mod, write_cif_vars_mon,
!   write_cif_vars_morph, write_cif_vars_nstspc, write_cif_vars_part,
!   write_cif_vars_partspc, write_cif_vars_sed, write_cif_vars_sedspc,
!   write_cif_vars_tspart, write_cif_vars_tsr, write_dar_spec, write_grid,
...
!   check_partition, check_part_params, check_sed_params, check_sed_spec,
!   check_sedics, check_structs_locs, close_filepars, default_bio_params,
!   default_bioics, default_dar_params, default_mod_params, default_morphics,
...
!   reset_part_params, reset_partics, reset_sed_params, reset_sed_spec,
!   read_sedics, rng_init, sum_vars
!
!************************************************************************
!
USE iopars
...
USE reset_sediments, ONLY: reset_dar_params, reset_morph_params, reset_sedics, &
                         & reset_sed_params, reset_sed_spec
...
!! CALL allocate_struc_arrays !! AC 07122022 -> moving it a few lines below to have nloc defined. 
...
!! AC 01122022 - Could include a few IFs here.. 
...
IF (iopt_sed.GT.0) THEN
   IF (iopt_sed.EQ.1) THEN
      ...
      IF (cifstatus(icif_sedspc)) THEN
         CALL read_cif_params(icif_sedspc)
      ...
      ELSEIF (modfiles(io_sedspc,1,1)%status.EQ.'R') THEN
         CALL read_sed_spec
      ENDIF
   ENDIF
   CALL reset_sed_spec
   IF (iopt_sed.EQ.1) CALL check_sed_spec
ENDIF

!
!6.2 Dredging and relocation
!----------------------------
!
...
IF (iopt_dischr.EQ.1) THEN
   IF (cifstatus(icif_disspc)) THEN
      CALL read_cif_params(icif_disspc)
   ELSE
...
IF (iopt_part_model.LE.2) THEN
   ...
   IF (cifstatus(icif_partspc)) THEN
      CALL read_cif_params(icif_partspc)
   ...
   ELSEIF (modfiles(io_parspc,1,1)%status.EQ.'R') THEN
      CALL read_part_spec
   ENDIF
ENDIF
...
!
!---2-D mode
IF (iopt_obc_2D.EQ.1) CALL define_2dobc_spec(io_2uvobc)
IF (iopt_obc_2D_tang.EQ.1) CALL define_2dobc_spec(io_2xyobc)

!---currents
IF (iopt_grid_nodim.EQ.3.AND.iopt_mode_3D.EQ.2) CALL current_corr
IF (iopt_obc_3D_tang.EQ.1) CALL update_tang_3dobc_data
...
!
!6.6 Gridded structures (MPV)
!----------------------------
!
!---MPV
IF (iopt_mpvcov.EQ.1) THEN
      IF (modfiles(io_mpvcov,1,1)%status.EQ.'R') THEN
      CALL read_MPV
   ELSEIF (modfiles(io_mpvcov,1,1)%status.EQ.'N') THEN
      CALL usrdef_MPV  
   ENDIF
   IF (modfiles(io_mpvcov,1,2)%status.EQ.'W') WRITE(*,*) "subroutine write_mpv hasn't been implemented yet" !CALL write_MPV ! .. not implemented yet .. 
ENDIF
...
!  ---intialise physical arrays
   CALL define_particle_phys_data(CDateTime)
...
!
!12. Send/write data for the particle model
!------------------------------------------
!
!---model grid
...
!
!13. Locations for nested output
!-------------------------------
!
...
!
!14. Initialise forcing and open boundary conditions
!---------------------------------------------------
!
!---1-D mode
...
!---tangential open boundary data
IF (iopt_obc_2D_tang.EQ.1) CALL update_tang_2dobc_data
IF (iopt_obc_3D_tang.EQ.1) CALL update_tang_3dobc_data
...
!  ---nesting parameters
   IF (iopt_nests.EQ.1) CALL write_cif_vars_nstspc
!  ---discharge module
   IF (iopt_dischr.EQ.1) CALL write_cif_vars_disspc
!  ---time series output 
   IF (iopt_out_tsers.GT.0) CALL write_cif_vars_tsr
!  ---time averaged output 
   IF (iopt_out_avrgd.GT.0) CALL write_cif_vars_avr
!  ---harmonic analysis
   IF (iopt_out_anal.GT.0) CALL write_cif_vars_freq 
!  ---harmonic output
   IF (iopt_out_anal.GT.0) CALL write_cif_vars_anal
!  ---sediment model parameters 
   IF (iopt_sed.GT.0) CALL write_cif_vars_sed
!  ---sediment model specifiers
   IF (iopt_sed.EQ.1) CALL write_cif_vars_sedspc
...
!  ---biological model parameters
   ...
   IF (iopt_part_model.EQ.1.OR.iopt_part_model.EQ.2) THEN
!     ---particle model parameters
      CALL write_cif_vars_part
!     ---particle model specifiers
      CALL write_cif_vars_partspc
!     ---particle output
...
IF (iopt_bstres_drag.GT.2.AND.iopt_bstres_drag.LT.5) THEN
...
!*Local variables
!
LOGICAL :: fcomm, header
CHARACTER (LEN=lentime) :: ciodatetime
INTEGER ::  ivar, nocoords1, nocoords2, numvars, numvars1, numvars2
...
header = modfiles(io_inicon,ics_phys,1)%header

IF (header) THEN

!  ---open data file
   filepars1 = modfiles(io_inicon,ics_phys,1)
...
!  ---variable attributes
   ...
   numvars1 = nocoords1 + filepars1%novars
...
!
!---file attributes
filepars2 = modfiles(io_inicon,ics_phys,1)
CALL set_modfiles_atts(io_inicon,ics_phys,1,filepars2)

!---data attributes
nocoords2 = filepars2%nocoords
numvars2 = nocoords2 + filepars2%novars
ALLOCATE (varatts2(numvars2),STAT=errstat)
CALL error_alloc_struc('varatts2',1,(/numvars2/),'VariableAtts')
CALL varatts_init(varatts2)
CALL set_modvars_atts(io_inicon,ics_phys,1,filepars2,numvars2,varatts2)
...
IF (header) THEN
   CALL error_value_arr_struc(nocoords1,'modfiles','nocoords',nocoords2,3,&
                           & (/io_inicon,ics_phys,1/))
   CALL check_value_varatts(io_inicon,ics_phys,varatts1,varatts2,filepars1,&
                          & filepars2)
   CALL error_abort('read_phsics',ierrno_read)
ENDIF
...
!     ---time
      CASE ('time')
         CALL read_time(ciodatetime,filepars,(/varatts(ivar)/))
...
!     ---X-current
      CASE ('udvel')
         ...
         CALL read_distribute_mod(udvel,filepars,ivar,lbounds(1:2),&
                                & ubounds(1:2),nhdims,'U  ',fcomm,&
                                & (/varatts(ivar)/))
...
!     ---Y-current
      CASE ('vdvel')
         ...
         lbounds(1:2) = (/1-nhalo,1-nhalo/)
         ubounds(1:2) = (/nc+nhalo,nr+nhalo/)
         CALL read_distribute_mod(vdvel,filepars,ivar,lbounds(1:2),&
                                & ubounds(1:2),nhdims,'V  ',fcomm,&
                                & (/varatts(ivar)/))
...
!     ---surface elevation
      CASE ('zeta')
         ...
         CALL read_distribute_mod(zeta,filepars,ivar,lbounds(1:2),&
...
!     ---X-current
      CASE ('uvel')
         ...
         CALL read_distribute_mod(uvel,filepars,ivar,lbounds,ubounds,nhdims,&
                               & 'U  ',fcomm,(/varatts(ivar)/))

!     ---Y-current
      CASE ('vvel')
         nhdims = nhalo
         lbounds = (/1-nhalo,1-nhalo,1/); ubounds = (/nc+nhalo,nr+nhalo,nz/)
         CALL read_distribute_mod(vvel,filepars,ivar,lbounds,ubounds,nhdims,&
                               & 'V  ',fcomm,(/varatts(ivar)/))

!     ---vertical current
      CASE ('wvel')
         nhdims = (/1,0,1,0/)
         lbounds = (/0,0,1/); ubounds = (/nc,nr,nz+1/)
         CALL read_distribute_mod(wvel,filepars,ivar,lbounds,ubounds,nhdims,&
                               & 'W  ',fcomm,(/varatts(ivar)/))
...
!     ---temperature
      CASE ('temp')
         nhdims = nhalo
         lbounds = (/1-nhalo,1-nhalo,1/); ubounds = (/nc+nhalo,nr+nhalo,nz/)
         CALL read_distribute_mod(temp,filepars,ivar,lbounds,ubounds,nhdims,&
                               & 'C  ',fcomm,(/varatts(ivar)/))

!     ---salinity
      CASE ('sal')
         nhdims = nhalo
         lbounds = (/1-nhalo,1-nhalo,1/); ubounds = (/nc+nhalo,nr+nhalo,nz/)
         CALL read_distribute_mod(sal,filepars,ivar,lbounds,ubounds,nhdims,&
                               & 'C  ',fcomm,(/varatts(ivar)/))
...
!     ---turbulence energy
      CASE ('tke')
         nhdims = nhalo
         lbounds = (/1-nhalo,1-nhalo,1/); ubounds = (/nc+nhalo,nr+nhalo,nz+1/)
         CALL read_distribute_mod(tke,filepars,ivar,lbounds,ubounds,nhdims,&
                               & 'W  ',fcomm,(/varatts(ivar)/))
...
!     ---mixing length
      CASE ('zlmix')
         nhdims = nhalo
         lbounds = (/1-nhalo,1-nhalo,1/); ubounds = (/nc+nhalo,nr+nhalo,nz+1/)
         CALL read_distribute_mod(zlmix,filepars,ivar,lbounds,ubounds,&
                                & nhdims,'W  ',fcomm,(/varatts(ivar)/))

!     ---dissipation rate
      CASE ('dissip')
         nhdims = nhalo
         lbounds = (/1-nhalo,1-nhalo,1/); ubounds = (/nc+nhalo,nr+nhalo,nz+1/)
         CALL read_distribute_mod(dissip,filepars,ivar,lbounds,ubounds,&
                                & nhdims,'W  ',fcomm,(/varatts(ivar)/))
...
!     ---bottom drag coefficient
      CASE ('bdragcoefatc')
         ...
         CALL read_distribute_mod(bdragcoefatc,filepars,ivar,lbounds(1:2),&
                                & ubounds(1:2),nhdims,'C  ',fcomm,&
                                & (/varatts(ivar)/))
...
!     ---roughness length
      CASE ('zroughatc')
         nhdims = (/1,0,1,0/)
         lbounds(1:2) = 0; ubounds(1:2) = (/nc,nr/)
         CALL read_distribute_mod(zroughatc,filepars,ivar,lbounds(1:2),&
                                & ubounds(1:2),nhdims,'C  ',fcomm,&
                                & (/varatts(ivar)/))
...
!     ---tidal phases
      CASE ('phase_obc')
         CALL read_vars(phase_obc,filepars,ivar,(/varatts(ivar)/))
...
!
      ...
      CASE ('wbarlossu')
         CALL read_vars(wbarelossu,filepars,ivar,(/varatts(ivar)/))
      CASE ('wbarlossv')         
         CALL read_vars(wbarelossv,filepars,ivar,(/varatts(ivar)/))
...
!     ---salinity
      CASE ('obcsalatu')
         CALL read_vars(obcsalatu,filepars,ivar,(/varatts(ivar)/))
      CASE ('obcsalatv')         
         CALL read_vars(obcsalatv,filepars,ivar,(/varatts(ivar)/))
         
!     ---temperature
      CASE ('obctmpatu')
         CALL read_vars(obctmpatu,filepars,ivar,(/varatts(ivar)/))
      CASE ('obctmpatv')         
         CALL read_vars(obctmpatv,filepars,ivar,(/varatts(ivar)/))
         
!     ---2-D mode
      CASE ('obc2uvatu')
         CALL read_vars(obc2uvatu,filepars,ivar,(/varatts(ivar)/))
      CASE ('obc2uvatv')
         CALL read_vars(obc2uvatv,filepars,ivar,(/varatts(ivar)/))

!     ---3-D current
      CASE ('obc3uvatu')
         CALL read_vars(obc3uvatu,filepars,ivar,(/varatts(ivar)/))
      CASE ('obc3uvatv')
         CALL read_vars(obc3uvatv,filepars,ivar,(/varatts(ivar)/))
...
! Author - Patrick Luyten
!
! Version - @(COHERENS)Model_Initialisation.F90  V2.12
!
...
USE inout_routines, ONLY: open_filepars, monitor_files, open_file, open_filepars
USE time_routines, ONLY: clock_date_time
USE utility_routines, ONLY: lower_case 
...
INTEGER :: i, icif, iend, ierr, iunit2, lvar, numvars
...
!
!---read input line
CALL read_cif_line(cline,cvals,numvars,1,filename='defruns')
CALL check_cif_lbound_novars(numvars,3,filename='defruns',&
                           & prname='simulation_start',abort=.TRUE.)
!---run title
runtitle = ''
lvar = 1
CALL conv_from_chars(cvals(lvar),runtitle,'runtitle',lvar)
...
ciffile%status = '0'
lvar = 2
CALL conv_from_chars(cvals(lvar),ciffile%status,'status',lvar)
...
!---name of CIF file
lvar = 3
CALL conv_from_chars(cvals(lvar),ciffile%filename,'filename',lvar)
...
!---status of cif blocks
cifstatus = .FALSE.
IF (ciffile%status.EQ.'R') THEN
   iunit2 = ciffile%iunit
   icif_510: DO icif=1,MaxCIFTypes
      REWIND(iunit2)
      DO WHILE (.NOT.cifstatus(icif))
         READ (iunit2,*,END=99) cline
         CALL lower_case(cline)
         IF (TRIM(ADJUSTL(cline)).EQ.TRIM(ADJUSTL(cifblocks(icif)))) THEN
            cifstatus(icif) = .TRUE.
         ENDIF
      ENDDO
99    CONTINUE
